...
### База PostgreSQL в Minikube (Helm)
1. Установите Helm (`brew install helm`) и добавьте репозиторий: `helm repo add bitnami https://charts.bitnami.com/bitnami && helm repo update`.
...
3. Обновите секрет приложения под новую БД:  
   ```
   ...
   ```
4. Примените миграции: `kubectl delete job django-migrate --ignore-not-found && kubectl apply -f kubernetes/django-migrate-job.yaml && kubectl wait --for=condition=complete job/django-migrate --timeout=120s && kubectl logs job/django-migrate`.
5. Перезапустите приложение: `kubectl rollout restart deploy/django-web`.
